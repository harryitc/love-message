...
import { Navigation, Coffee, Heart, Home, ArrowRight, Loader2, MapPin, Send, MessageCircle } from 'lucide-react';
...
const ShipLove = ({ playSFX }) => {
  ...
  const [form, setForm] = useState({ drink: '', message: '' });
  ...
  const updateMap = (lat, lng, path) => {
    ...
    if (path && path.length > 1) {
      if (!pathRef.current) pathRef.current = L.polyline(path, { color: '#ffb6c1', weight: 4, dashArray: '10, 10' }).addTo(mapRef.current);
      else pathRef.current.setLatLngs(path);
    }
          ...
          {mode === 'ship' ? (
            !activeShipment?.active ? (
              <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="bg-white rounded-[2.5rem] p-6 shadow-2xl border-2 border-pink-100 space-y-4">
                <div className="flex items-center gap-3 mb-2"><div className="p-2 bg-purple-100 rounded-xl text-purple-600"><Coffee size={20} /></div><h2 className="font-bold text-slate-800">Chuẩn bị chuyến xe tình yêu</h2></div>
                <div className="space-y-3">
                  <input type="text" placeholder="Em ấy uống gì thế?" value={form.drink} onChange={(e) => setForm({...form, drink: e.target.value})} className="w-full px-5 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-pink-300 outline-none transition-all text-sm" />
              ...
              <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-slate-900 text-white rounded-[2.5rem] p-6 shadow-2xl space-y-4">
                <div className="flex justify-between items-center gap-4">
                  <div className="flex items-center gap-3"><Loader2 className="animate-spin text-pink-400" size={20} /><div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Đang cập nhật vị trí...</p><p className="text-sm font-bold">Shipping: {activeShipment.drink}</p></div></div>
